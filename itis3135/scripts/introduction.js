document.addEventListener("DOMContentLoaded", function () {
  ...
  function generateIntro() {
    ...
    const personalStatement = document.getElementById("personal-statement").value.trim();
    ...
    const primaryComputer = document.getElementById("primary-computer").value.trim();
    const backupComputer  = document.getElementById("backup-computer").value.trim();
    ...
    }).filter(function (c) { return c.dept || c.num || c.name; });
    ...
    const displayName = nickname ? nameParts + " (" + nickname + ")" : nameParts;
    const heading = displayName + " " + divider + " " + mascotAdj + " " + mascotAnimal;

    // Build courses list HTML
    const coursesHTML = courses.map(function (c) {
      return "<li><b>" + c.dept + " " + c.num + " - " + c.name + ":</b> " + c.reason + "</li>";
    }).join("\n            ");

    // Build the intro HTML matching the structure of introduction.html exactly
    function buildHTML(imgSrc) {
      return (
        ...
        "  <li><b>Background in Subject:</b> " + subjectBg + "</li>\n" +
        "  <li><b>Primary Work Computer &amp; Location:</b> " + primaryComputer + "</li>\n" +
        "  <li><b>Backup Work Computer &amp; Location Plan:</b> " + backupComputer + "</li>\n" +
        "  <li>\n" +
        "    <b>Current Courses:</b>\n" +
        "    <ol>\n" +
        "      " + coursesHTML + "\n" +
        "    </ol>\n" +
        "  </li>\n" +
        (funnyThing ? "  <li><b>Fun Thing About Me:</b> " + funnyThing + "</li>\n" : "") +
        (share      ? "  <li><b>I'd also like to share:</b> " + share + "</li>\n" : "") +
        "</ul>\n" +
        "<blockquote>\n" +
        "  &ldquo;" + quote + "&rdquo; <br /><cite>- " + quoteAuthor + "</cite>\n" +
        "</blockquote>"
      );
    }
    ...
    function renderIntro(imgSrc) {
      document.querySelector('link[rel="stylesheet"]').href = "../styles/default.css";
      ...
      Promise.all([
        ...
        // Fix relative links in the injected header so they resolve from itis3135/
        ...
        if (links.length > 0) {
          ...
          if (footerNav) {
            footerNav.innerHTML = links.map(function (l) {
              return "<a href=\"" + l.url + "\">" + l.label + "</a>";
            }).join("\n    |\n    ");
          }
        }

        // Append reset link to footer so it doesn't affect main content comparison
    ...
    if (pictureFile) {
      ...
      reader.onload = function (e) { renderIntro(e.target.result); };
  ...
  // Reset: native button reset handles form values; this clears any validation outlines
  document.querySelector("button[type='reset']").addEventListener("click", function () {
    Array.from(form.querySelectorAll("input, textarea")).forEach(function (field) {
      field.style.outline = "";
    });
  });

  // Add Course: dynamically append a new course fieldset
  let courseCount = document.querySelectorAll("#intro-form .course-entry").length;

  function addCourse() {
    courseCount++;
    const coursesContainer = document.querySelector("#intro-form fieldset:has(#add-course)");
    const newCourse = document.createElement("fieldset");
    newCourse.classList.add("course-entry");
    newCourse.innerHTML =
      "<legend>Course " + courseCount + "</legend>" +
      "<label for=\"course" + courseCount + "-dept\">Department:</label>" +
      "<input type=\"text\" id=\"course" + courseCount + "-dept\" name=\"course" + courseCount + "-dept\" placeholder=\"e.g. ITIS\" />" +
      "<label for=\"course" + courseCount + "-num\">Course Number:</label>" +
      "<input type=\"text\" id=\"course" + courseCount + "-num\" name=\"course" + courseCount + "-num\" placeholder=\"e.g. 3135\" />" +
      "<label for=\"course" + courseCount + "-name\">Course Name:</label>" +
      "<input type=\"text\" id=\"course" + courseCount + "-name\" name=\"course" + courseCount + "-name\" />" +
      "<label for=\"course" + courseCount + "-reason\">Reason:</label>" +
      "<textarea id=\"course" + courseCount + "-reason\" name=\"course" + courseCount + "-reason\"></textarea>";
